...
Multi-layer Cascade Early Exit with Confidence Accumulation for vLLM

This module patches vLLM's Qwen2 model to enable dynamic early exit
based on accumulated confidence across multiple checkpoint layers.

Key Innovations in V6.1:
    - Confidence Accumulation: Cross-layer aggregation instead of single-point decisions
    - Soft Exit: Gradual MLP computation reduction (not hard skip)
    - Rust Core: High-performance batch evaluation (optional, with Python fallback)

Environment Variables:
    SEDAC_ENABLED           : Enable/disable SEDAC (0/1)
    SEDAC_PROBE_LAYERS      : Comma-separated checkpoint layers (default: 7,14,21)
    SEDAC_THRESHOLDS        : Initial thresholds per layer (default: 0.8,1.0,1.2)
    SEDAC_CONFIDENCE_DECAY  : Decay factor for accumulation (default: 0.9)
    SEDAC_SOFT_EXIT         : Enable soft exit mode (0/1, default: 1)
    SEDAC_LAYER_WEIGHTS     : Per-layer confidence weights (default: 0.3,0.4,0.3)
    SEDAC_ALPHA             : EMA smoothing factor (default: 0.1)
    SEDAC_PROBE_DIR         : Directory containing trained probe weights
    SEDAC_PROBE_RANK        : Probe hidden dimension (default: 64)

Reference:
    SEDAC V6.1 Design Document: docs/SEDAC_V6.1_DESIGN.md
...
# V6.1: Try to import Rust core, fallback to Python
_SEDAC_RUST_AVAILABLE = False
_sedac_cascade_controller = None

try:
    from sedac_core import CascadeController, LayerConfig, ExitDecision
    _SEDAC_RUST_AVAILABLE = True
except ImportError:
    pass
"""
...
init_code_v61 = """
        ...
        # V6.1: Multi-layer configuration
        ...
        # V6.1: Confidence accumulation parameters
        self.sedac_confidence_decay = float(os.environ.get("SEDAC_CONFIDENCE_DECAY", "0.9"))
        self.sedac_soft_exit = os.environ.get("SEDAC_SOFT_EXIT", "1").lower() in ("1", "true", "yes")
        ...
        # V6.1: Per-token state for confidence accumulation
        ...
        self._sedac_logger.warning(
            "SEDAC patch v6.1 (confidence accumulation) enabled=%s layers=%s thresholds=%s soft_exit=%s",
            self.sedac_enabled, self.sedac_probe_layers, self.sedac_thresholds, self.sedac_soft_exit,
        )
            ...
            try:
                ...
                if pp_world_size != 1:
                    ...
                    for _layer_idx in self.sedac_probe_layers:
                        ...
                        self._sedac_logger.warning(
                            "SEDAC probe loaded: layer=%d threshold=%.4f weight=%.2f", 
                            _layer_idx, self.sedac_thresholds[_layer_idx], self.sedac_layer_weights[_layer_idx]
                        )
                        _loaded_any = True
                    ...
                    if not _loaded_any:
                        ...
                        self._sedac_logger.warning("SEDAC V6.1 ready: %d probes loaded, rust=%s", len(self.__dict__["sedac_probes"]), _SEDAC_RUST_AVAILABLE)
            except Exception:
                self.sedac_enabled = False
                self._sedac_logger.exception("SEDAC probe load failed")
        self._sedac_patch_end = 61
"""
            ...
            if self.sedac_enabled:
                ...
                if _abs_layer in _probes:
                    ...
                    with torch.inference_mode():
                        ...
                        _risk = _probe(current_h).squeeze(-1)  # [batch, seq] -> take last token
                        
                        # For decoding, typically seq_len=1, but handle prefill too
                        if _risk.dim() > 1:
                            _risk = _risk[:, -1]  # [batch]
                        ...
                        # V6.1: Compute layer confidence (how much below threshold)
                        _layer_confidence = ((_threshold - _risk) / (_threshold + 1e-6)).clamp(0, 1)
                        
                        # V6.1: Accumulate confidence with decay
                        ...
                        # V6.1: Check exit condition (only for non-exited tokens)
                        ...
                        if _can_exit.any():
                            ...
                            if self._sedac_exits_metric is not None:
                                try:
                                    self._sedac_exits_metric.inc(_exit_count)
                                except Exception:
                                    pass
                            
                            # V6.1: Compute soft exit ratios
                            if self.sedac_soft_exit:
                                # tanh mapping for smooth transition
                            ...
                            # Mark subsequent layers for MLP skip/reduction
                        ...
                        if self.sedac_log_every > 0 and (self.sedac_calls % self.sedac_log_every) == 0:
                            _avg_conf = self._sedac_accumulated_confidence.mean().item()
                            _exit_pct = self._sedac_exited_mask.float().mean().item() * 100
                            self._sedac_logger.warning(
                                "SEDAC L%d avg_conf=%.3f exit_pct=%.1f%% thr=%.4f calls=%d exits=%s",
                                _abs_layer, _avg_conf, _exit_pct, self.sedac_thresholds[_abs_layer],
                                self.sedac_calls, self.sedac_exit_calls,
                            )
...
def _sedac_decoder_forward(
    ...
    if _skip_mlp and _soft_ratio is not None and _exited_mask is not None:
        # V6.1: Soft exit - partial MLP computation
        if residual is None:
        ...
        # Always run attention (KV cache consistency)
        hidden_states = self.self_attn(positions=positions, hidden_states=hidden_states)
        hidden_states, residual = self.post_attention_layernorm(hidden_states, residual)
        
        # Soft exit: blend full MLP with zero output based on soft_ratio
        _batch_size = hidden_states.shape[0]
        if _batch_size == _soft_ratio.shape[0]:
            # Compute full MLP
            mlp_out = self.mlp(hidden_states)
            
            # Apply soft masking per token
            # _soft_ratio: [batch], _exited_mask: [batch]
            # For exited tokens: output = mlp_out * (1 - soft_ratio)
            # For non-exited tokens: output = mlp_out
            _mask = _exited_mask.view(-1, 1, 1).float()  # [batch, 1, 1]
            _ratio = _soft_ratio.view(-1, 1, 1)  # [batch, 1, 1]
            
            # Blend: exited tokens get reduced MLP, others get full
            hidden_states = mlp_out * (1.0 - _mask * _ratio)
        else:
            # Dimension mismatch fallback: skip MLP entirely
            hidden_states = hidden_states.new_zeros(hidden_states.shape)
        
        return hidden_states, residual
    
    elif _skip_mlp:
        # Hard exit fallback (V6.0 behavior)
        if residual is None:
            residual = hidden_states
            hidden_states = self.input_layernorm(hidden_states)
        else:
...
def _strip_sedac_blocks(lines: list[str]) -> list[str]:
    ...
    while i < len(lines):
        ...
        if stripped in ("# --- SEDAC DECODER PATCH ---", "# --- SEDAC DECODER PATCH v2 ---", "# --- SEDAC DECODER PATCH v6 ---", "# --- SEDAC DECODER PATCH v6.1 ---"):
             i += 1
             while i < len(lines):
                 if lines[i].strip() == "# ---------------------------":
                     i += 1
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SEDAC V6.1 vLLM Patcher")
    ...
    print(f"SEDAC V6.1 Patcher (Confidence Accumulation + Soft Exit)")
    ...
    for i, line in enumerate(lines):
        ...
        if not inserted_init and "self.aux_hidden_state_layers = tuple[int, ...]()" in line:
            new_lines.append(init_code_v61)
            inserted_init = True
        ...
        if not inserted_forward and "hidden_states, residual = layer(positions, hidden_states, residual)" in line:
            new_lines.append(forward_code_v61)
            inserted_forward = True

    # Append Decoder Patch at the end
    new_lines.append(decoder_patch_v61)
    ...
    if args.dry_run:
        ...
        print("✅ Successfully patched with SEDAC V6.1!")
        print("\nKey V6.1 Features:")
        print("  - Confidence Accumulation across layers")
        print("  - Soft Exit (gradual MLP reduction)")
        print("  - EMA threshold smoothing")
        print("\nUsage:")
        print("  export SEDAC_ENABLED=1")
        print("  export SEDAC_PROBE_LAYERS=7,14,21")
        print("  export SEDAC_CONFIDENCE_DECAY=0.9")
        print("  export SEDAC_SOFT_EXIT=1")
